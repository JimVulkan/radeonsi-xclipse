...
{
   /* Samsung sgpu (Xclipse, 0x73a0): the FENCE chunk makes the kernel GART-bind the user fence BO
    * at submit (amdgpu_cs_parser_bos -> amdgpu_ttm_alloc_gart), and every GART bind on this chip
    * runs a raw-MMIO TLB flush that can take the whole phone down. It is the only CS-time GART bind
    * a graphics submit can reach. The vendor driver never sends the chunk; neither does RADV here.
    * Without it amdgpu_fence_wait() asks the kernel instead of reading the fence memory. */
   if (acs->aws->info.pci_id == 0x73a0)
      return false;
...
static void amdgpu_cs_submit_ib(void *job, void *gdata, int thread_index)
{
   ...
   for (i = 0; i < initial_num_real_buffers; i++) {
      ...
      if (queue_type != USERQ) {
         if (!get_real_bo(buffer->bo)->vm_always_valid && !get_real_bo(buffer->bo)->resident_off_list)
