...
struct amdgpu_bo_real {
   ...
   /* Samsung sgpu (0x73a0): an imported dma-buf the kernel keeps resident on its own. Never put in
    * a submit's BO list, like the vendor driver and RADV on this chip: listing one made the kernel
    * validate it at submit, on a kernel where a cold bind there can reset the phone. */
   bool resident_off_list;
};
