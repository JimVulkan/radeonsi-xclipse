                           ...
                           int fd)
{
   ...
   /* Samsung sgpu (0x73a0): the vendor driver creates every private buffer VM_ALWAYS_VALID, so its
    * submits list only shared buffers. Do the same: fewer buffers for the kernel to validate on
    * every submit, on a kernel where anything that binds GART space at submit is a hazard. */
   if (aws->info.pci_id == 0x73a0)
      aws->info.has_vm_always_valid = true;
