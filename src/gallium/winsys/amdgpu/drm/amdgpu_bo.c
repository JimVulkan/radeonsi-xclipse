                                                 ...
                                                 int heap)
{
   ...
   /* Samsung Xclipse: no VRAM at all (vram_size 0), although the chip is treated as having
    * dedicated VRAM for the heap layout. Place VRAM requests in GTT, write-combined like a VRAM
    * mapping would be: with a cached CPU mapping, shader code written with memcpy() is not seen by
    * the GPU's instruction fetch. */
   if (!aws->info.vram_size_kb && (initial_domain & RADEON_DOMAIN_VRAM)) {
   ...
   if (initial_domain & RADEON_DOMAIN_VRAM_GTT && !(flags & RADEON_FLAG_NO_VMA)) {
      ...
      /* Samsung sgpu (0x73a0): map everything like the vendor driver, R|W|X and no MTYPE. */
      if (flags & RADEON_FLAG_GL2_BYPASS && aws->info.pci_id != 0x73a0)
                                               ...
                                               bool is_prime_linear_buffer)
{
   ...
   r = amdgpu_bo_va_op_common(aws, NULL, kms_handle, false, &bo->vm_timeline_point, 0,
                              result.alloc_size, va, AMDGPU_VM_PAGE_READABLE |
                                 ...
                                 (is_prime_linear_buffer && aws->info.pci_id != 0x73a0 ?
   ...
   bo->kms_handle = kms_handle;
   bo->is_shared = true;
   bo->resident_off_list = aws->info.pci_id == 0x73a0;
                                    ...
                                    uint64_t va)
{
   ...
   if (buf->usage & RADEON_FLAG_GL2_BYPASS && aws->info.pci_id != 0x73a0)
      vm_flags |= AMDGPU_VM_MTYPE_UC;
