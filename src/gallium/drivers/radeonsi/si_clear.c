                          ...
                          const union pipe_color_union *color, float depth, uint8_t stencil)
{
   ...
   while (color_buffer_mask) {
      ...
      if (vi_dcc_enabled(tex, level)) {
         ...
         /* No CMASK on GFX11. Nor on Xclipse 920: its GFX11 texture unit can't read CMASK fast
          * clears, so fast-cleared targets would sample black and drop colour writes. */
