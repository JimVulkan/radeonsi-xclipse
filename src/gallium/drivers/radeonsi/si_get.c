...
void si_init_screen_caps(struct si_screen *sscreen)
{
   ...
   /* Xclipse has no VRAM (vram_size 0); report the GTT that backs everything instead of 0 MB. */
   caps->video_memory = (sscreen->info.vram_size_kb ? sscreen->info.vram_size_kb
                                                    : sscreen->info.gart_size_kb) >> 10;
