...
static void si_emit_cb_render_state(struct si_context *sctx, unsigned index)
{
   ...
   if (sctx->screen->info.rbplus_allowed) {
       ...
       *
       * Not on Xclipse 920, where a zero downconvert is legitimate (as in RADV).
       */
      if (!sx_ps_downconvert && !sctx->screen->info.gfx11_shader_core)
                                        ...
                                        const struct pipe_blend_state *state, unsigned mode)
{
   ...
   if (sctx->screen->info.rbplus_allowed) {
       ...
       * Vulkan does this.
       *
       * Xclipse 920: always. The optimisation hints make its CB skip real destination reads, so
       * colour writes never land. RB+ itself stays on.
       */
      if (blend->dual_src_blend || sctx->screen->info.gfx11_shader_core) {
         for (int i = 0; i < num_shader_outputs; i++) {
...
static void si_emit_db_render_state(struct si_context *sctx, unsigned index)
{
   ...
   /* This should always be set on GFX11, and on Xclipse 920, whose pixel pipe counts the GFX11
    * way. */
   if (sctx->gfx_level >= GFX11 || sctx->screen->info.gfx11_shader_core)
      db_count_control |= S_028004_DISABLE_CONSERVATIVE_ZPASS_COUNTS(1);
...
static bool si_is_sampler_format_supported(struct pipe_screen *screen, enum pipe_format format)
{
   ...
   /* Xclipse 920: the texture unit decodes BC1-BC3 only. Without BC4-BC7 here, st/mesa decodes
    * RGTC/LATC/BPTC on upload. */
   if (sscreen->info.gfx11_shader_core &&
       (desc->layout == UTIL_FORMAT_LAYOUT_RGTC || desc->layout == UTIL_FORMAT_LAYOUT_BPTC))
      return false;

   if (sscreen->info.gfx_level >= GFX10) {
      const struct gfx10_format *fmt =
         &ac_get_gfx10_format_table(sscreen->info.desc_gfx_level)[format];
                                              ...
                                              unsigned usage)
{
   ...
   if (sscreen->info.gfx_level >= GFX10) {
      const struct gfx10_format *fmt =
         &ac_get_gfx10_format_table(sscreen->info.desc_gfx_level)[format];
      unsigned first_image_only_format = sscreen->info.desc_gfx_level >= GFX11 ? 64 : 128;
                               ...
                               uint32_t *state)
{
   ...
   ac_build_buffer_descriptor(screen->info.desc_gfx_level, &buffer_state, &state[0]);
}
                                       ...
                                       const struct pipe_vertex_element *elements)
{
   ...
   for (i = 0; i < count; ++i) {
      ...
      ac_set_buf_desc_word3(sscreen->info.desc_gfx_level, &buffer_state, &v->elem[i].rsrc_word3);
   }
                                            ...
                                            struct si_pm4_state *pm4)
{
   ...
   /* Xclipse 920: CB/DB use the kernel clear-state GL2 policy (stream writes, no-allocate reads),
    * as in RADV on this chip. */
   const bool xclipse = sscreen->info.gfx11_shader_core;

   const struct ac_preamble_state preamble_state = {
      .border_color_va = border_color_va,
      .gfx10.cache_cb_gl2 = sctx->gfx_level >= GFX10 && sscreen->options.cache_cb_gl2 && !xclipse,
      .gfx10.cache_db_gl2 = sctx->gfx_level >= GFX10 && sscreen->options.cache_db_gl2 &&
                            !is_process_name_param("GpuTest", "fur") && !xclipse,
   };

   ac_init_graphics_preamble_state(&preamble_state, &pm4->base);

   /* Xclipse IFPO guard: the sgpu kernel's inter-frame power-off can wedge a CPU in its power-on
    * busy-wait until the watchdog resets the phone. It does not power off while CP_PERFMON_CNTL
    * bits [7:0] are non-zero, so keep the perfmon block in STOP_COUNTING, re-armed by every
    * submit's preamble. Same as RADV on this chip. */
   if (xclipse) {
      ac_pm4_set_reg(&pm4->base, R_036020_CP_PERFMON_CNTL,
